    ...
    if(isIndPage || pathURL === '/user/login'){ //此处单独判断登入页，是为了兼容旧版（即未在 config.js 配置 indPage 的情况）
      ...
      if(setter.interceptor){
        ...
        if(!local[setter.request.tokenName]){
          return location.hash = '/user/handleLogin/redirect='+ encodeURIComponent(pathURL); //跳转到登入页
        }
      }
      ...
      if(admin.pageType === 'console') { //后台主体页
        ...
        //自己加的，获取用户id，类型（权限），名字
        $.get(layui.setter.host+'AppAdmin/Apps.jsp',{act:'userInfo',token:layui.data('layuiAdmin').token},function(res) {
          var userTemp = JSON.parse(res)
          layui.data('UserName', { key: 'UserName' ,value: userTemp.USER_NAME||userTemp.USER_ID })
          layui.data('userType', { key: 'userType' ,value: userTemp.USER_ROLE === 'app_admin'? 1 : 0  });//1:app管理员，0:权限管理员
          

          //自己写的，找了老半天才在admin.js研究出来，默认关闭侧边栏的方法
          $('#'+ setter.container).addClass('layadmin-side-shrink')

          //下面注释的复制
          container.render('layout',userTemp.USER_NAME).done(function(){
            renderPage();
        ...
        // container.render('layout',layui.data('UserName').UserName).done(function(){
        //   renderPage();
        //   layui.element.render();
